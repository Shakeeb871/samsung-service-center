...
<section class="hero">
  <div class="wrap">
    <div class="hero-grid">
      ...
      <div>
        <div class="rating">
          <span class="stars"><?= str_repeat(icon('star', 16), 5) ?></span>
        ...
        <div class="hero-actions">
          <a class="btn" href="tel:<?= BIZ_PHONE_LINK ?>"><?= icon('phone', 18) ?>Call Us</a>
          <a class="btn btn-ghost" href="#book">Book Now</a>
        </div>
      </div>

      <div>
        <ul class="hero-checks">
          <li><?= icon('check', 16) ?><span>24/7 customer service, day or night</span></li>
          <li><?= icon('check', 16) ?><span>Fast response within 1 hour on emergency call-outs</span></li>
          <li><?= icon('check', 16) ?><span>Experienced Samsung specialist dispatched to your door</span></li>
          <li><?= icon('check', 16) ?><span>Upfront quote on every repair, no hidden fees</span></li>
          <li><?= icon('check', 16) ?><span>Full 90 day warranty on repairing</span></li>
          <li><?= icon('check', 16) ?><span>Covering every state in the UAE</span></li>
        </ul>
      </div>
    ...
    <div class="enquiry-bar" id="book">
      <form id="enquiry-form" action="/api/contact.php" method="post">
        ...
        <select name="appliance" required aria-label="Service required">
          ...
          <option value="<?= strip_tags(html_entity_decode($s['short'])) ?>"><?= $s['short'] ?></option>
          <?php endforeach; ?>
  ...
  <div class="wrap">
    ...
    <div class="split">
      ...
      <div>
        ...
        <p>
          <a class="btn btn-dark" href="tel:<?= BIZ_PHONE_LINK ?>"><?= icon('phone', 18) ?>Speak to a specialist</a>
        </p>
      </div>
    </div>
    ...
    <div class="split">
      <div>
        ...
        <ul class="emirates">
          ...
          <li><?= icon('pin', 16) ?><?= htmlspecialchars($e) ?></li>
          <?php endforeach; ?>
        </ul>
      </div>
...
<section class="section section-grey">
  <div class="wrap split">
    ...
    <div class="callout">
      ...
      <a class="phone-big" href="tel:<?= BIZ_PHONE_LINK ?>"><?= icon('phone', 22) ?><?= htmlspecialchars(BIZ_PHONE) ?></a>
    </div>
  </div>
</section>
...
<section class="cta-band">
  <div class="wrap cta-inner">
    ...
    <div class="cta-actions">
      <a class="btn btn-white" href="tel:<?= BIZ_PHONE_LINK ?>"><?= icon('phone', 18) ?><?= htmlspecialchars(BIZ_PHONE) ?></a>
